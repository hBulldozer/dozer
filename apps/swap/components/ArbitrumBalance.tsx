...
export const ArbitrumBalance: FC<ArbitrumBalanceProps> = ({ token, className = '' }) => {
  const { connection, loadBalances } = useBridge()
  const [loading, setLoading] = useState(false)
  
  useEffect(() => {
    if (token?.bridged && connection.arbitrumConnected && token.originalAddress) {
      setLoading(true)
      // Load the token balance from Arbitrum
      loadBalances([token.originalAddress])
        .finally(() => setLoading(false))
    }
  }, [token, connection.arbitrumConnected, loadBalances])
  
  // If not a bridged token or not connected to Arbitrum, don't show anything
  if (!token?.bridged || !connection.arbitrumConnected) {
    return null
  }
  
  const balance = token.originalAddress ? connection.arbitrumBalance[token.originalAddress] || 0 : 0
    ...
    <div className={`flex items-center gap-1 text-xs text-blue-400 ${className}`}>
      <span className="flex items-center">
        ...
        {loading ? (
          ...
          <span>Arbitrum: {balance.toFixed(2)}</span>
        )}
      </span>
    </div>
  )
