...
export const NanoStateRouter = createTRPCRouter({
  byUser: procedure.input(z.object({ address: z.string() })).query(async ({ input }) => {
    const contractId = '00000076646da548460355a81b05f99ff4cefb2851f7de3e758a1a1ae3dbabb1 '
    const resp = await fetch(
      `http://localhost:8080/v1a/nano_contract/state?id=${contractId}&fields[]=earnings_per_second&fields[]=owner_balance&fields[]=user_deposits.a'${input.address}'`
    )
  ...
  api: procedure.input(z.object({})).query(async ({ ctx }) => {
