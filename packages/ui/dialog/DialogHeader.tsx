  ...
  return (
    <div
      className={classNames(
        className,
        border ? 'border-b border-stone-200/5' : '',
    ...
    >
      {onBack ? (
        <IconButton className="flex items-center justify-center w-6 h-6 gap-2 cursor-pointer" onClick={onBack}>
          <ChevronLeftIcon width={24} height={24} className="cursor-pointer text-stone-100 hover:text-stone-50" />
        </IconButton>
      ) : (
        <div />
      )}

      <Typography weight={500} as="h3" className="flex justify-center text-lg font-medium leading-6 text-stone-100">
        {title}
      </Typography>
      ...
      <div className="flex justify-end">
        {children ? (
          ...
          <IconButton className="cursor-pointer" onClick={onClose}>
            <XMarkIcon width={24} height={24} className="hover:text-stone-50 text-stone-100" />
          </IconButton>
        ) : (
          <span />
        )}
