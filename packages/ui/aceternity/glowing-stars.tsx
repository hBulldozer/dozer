...
export const GlowingStarsDescription = ({
  ...
  return <p className={classnames('text-base text-white max-w-[16rem]', className)}>{children}</p>
}

export const GlowingStarsTitle = ({ className, children }: { className?: string; children?: React.ReactNode }) => {
  return <h2 className={classnames('font-bold text-2xl text-[#eaeaea]', className)}>{children}</h2>
}
...
const Star = ({ isGlowing, delay }: { isGlowing: boolean; delay: number }) => {
  return (
    <motion.div
      ...
      className={classnames('bg-[#666] h-[1px] w-[1px] rounded-full relative z-20')}
    ></motion.div>
  )
}
